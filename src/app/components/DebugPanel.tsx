...
export function DebugPanel({ apiKey, useRealWeather, weather, error }: DebugPanelProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [testResults, setTestResults] = useState<{ api?: any; location?: any }>({});

  const maskApiKey = (key: string) => {
    if (key === 'YOUR_API_KEY_HERE' || key.length < 10) return key;
    return `${key.substring(0, 4)}...${key.substring(key.length - 4)}`;
  };

  const handleTestAPI = async () => {
    const result = await testOpenWeatherAPI(apiKey);
