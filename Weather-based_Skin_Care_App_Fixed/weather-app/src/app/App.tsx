...
function App() {
  ...
  const [weather, setWeather] = useState(getMockWeatherData('New York'));
  const [recommendations, setRecommendations] = useState(getSkincareRecommendations(weather));
  ...
  // Weather updates - real or simulated
  useEffect(() => {
    const interval = setInterval(async () => {
      if (useRealWeather) {
        // Refresh real weather data every 10 minutes
        try {
        ...
        // Simulate weather fluctuations for demo
        const currentWeather = getMockWeatherData(city);
        const updatedWeather = {
          ...currentWeather,
          temperature: currentWeather.temperature + Math.floor(Math.random() * 3) - 1,
          humidity: Math.min(100, Math.max(0, currentWeather.humidity + Math.floor(Math.random() * 5) - 2)),
          uvIndex: Math.min(11, Math.max(0, currentWeather.uvIndex + (Math.random() > 0.5 ? 1 : 0)))
        };
        setWeather(updatedWeather);
        setRecommendations(getSkincareRecommendations(updatedWeather));
      }
    }, useRealWeather ? 600000 : 30000); // 10 minutes for real, 30 seconds for demo
  ...
  const handleRefresh = async () => {
    ...
    try {
      const newWeather = useRealWeather
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl">SkinWeather</h1>
        ...
        {!useRealWeather && (
          <Alert className="mb-6 bg-blue-50 border-blue-200">
            ...
            <AlertDescription className="text-blue-900">
              Using demo weather data. Click <strong>"Use My Location"</strong> or search a city above to get live weather data.
            </AlertDescription>
          </Alert>
        )}
